...
// 对输出结果进行格式化
var responseFormat = require('./response-format.js');
...
function _getPageInfoByParams(params) {
	...
	var size = parseInt(params.size || '20', 10);
...
helper.connection = function(callback, opt) {
	var conn = mysql.createConnection(assign({
...
// sql 格式化
helper.sqlFormat = function(sql, values) {
...
// 拼接where字符串
helper.where = function(where, values){
...
// 进行数据库查询操作
helper.query = function(sql, values, callback) {
...
// 批量插入
helper.batInsert = function(table, values, callback) {
	var head,
...
// 批量查询
helper.batQuery = function(sqls, values, callback) {
	var sqlList = [];
...
*/
helper.insertRows = function(opt){
	var send;
